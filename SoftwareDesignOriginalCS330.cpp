 ...
 * Class:      CS-330-J6592 Comp Graphic and Visualization 20EW6
 * Assignment: 7-1 Final Submission
 */

/* This program uses modern OpenGL and the API libraries to draw
 * 2D and 3D objects with rotation effects, camera locations and
 * lighting effects. In this program, a 4-leg table object is drawn,
 * rotated, and lighted with 2 cameras.
 */
...
#define WINDOW_TITLE "7-1 Final Project (William Moore)"   // Macro for window title
...
const GLchar * fragmentShaderSource = GLSL(330,
    ...
    uniform vec3 objectColor; uniform vec3 keyLightColor; uniform vec3 keyLightPosition;
    uniform vec3 fillLightColor; uniform vec3 fillLightPosition; uniform vec3 viewPosition;
    ...
    float keyAmbientStrength = 0.4f;// Set ambient or global lighting strength
    float fillAmbientStrength = 0.6f;// Set ambient or global lighting strength
    ...
    //pyramidColor = texture(uTexture, mobileTextureCoordinate); // Sends texture to the GPU for rendering
    //gpuTexture = vec4(phong, 1.0f);                            // Send lighting results to GPU

    vec3 objectColor = mobileColor; vec3 phong = (ambient) * objectColor;
    //vec3 phong = (diffuse) * objectColor;
    //vec3 phong = (specular) * objectColor;
    //vec3 phong = (ambient + specular) * objectColor;
    //vec3 phong = (ambient + diffuse + specular) * objectColor;
    gpuColor = vec4(mobileColor, 1.0f);             //Send lighting results to GPU
...
int main(int argc, char* argv[]) {
    // initialize all the glut functions
    // -- set the display mode
    // -- initialize the window size
    // -- create the draw window with title
    glutInit(&argc, argv);
    ...
    // initialize the glew functionality
    ...
    // initialize the keyboard and mouse functions
    initializeKeyboard();
    initializeMouse();
...
void UCreateShader(void) {
    ...
    if (infologLength > 0) {
        infoLog = (char *) malloc(infologLength);
        glGetShaderInfoLog(fragmentShader, infologLength, &charsWritten,
                infoLog);
    ...
    if (infologLength > 0) {
        infoLog = (char *) malloc(infologLength);
        glGetShaderInfoLog(lampVertexShader, infologLength, &charsWritten,
                infoLog);
    ...
    if (infologLength > 0) {
        infoLog = (char *) malloc(infologLength);
        glGetShaderInfoLog(lampFragmentShader, infologLength, &charsWritten,
                infoLog);
...
void onMotion(int curr_x, int curr_y) {
    ...
    if (checkMotion) {
        ...
        if ((yaw != yaw + mouseXOffset) && (pitch == pitch + mouseYOffset)) {
            ...
            //else movement in y
        } else if ((pitch != pitch + mouseYOffset)
                && (yaw == yaw + mouseXOffset)) {
